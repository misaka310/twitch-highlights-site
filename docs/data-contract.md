...
`items[]` の `headline` は内部エンリッチメントの結果です。YouTubeで文字起こし不能などにより生成できない場合、公開UIは見出し未生成として扱い、`reason` を表示用見出しへ変換しません。YouTubeの `headline` 欠損は項目単位で許容し、更新全体を止めません。生成済み `headline` は公開品質検証を通過している必要があり、通過しない見出しは公開せず欠損として扱います。
