...
- Oracle素材を処理する `process-youtube-material.yml` の見出し生成は、Whisper文字起こしを入力としてGroqの `openai/gpt-oss-120b` を使う。LLM応答が得られない場合やローカル抽出フォールバックしか得られない場合、その見出しは公開せず更新処理を失敗させる。LLMの候補選定と再試行は公開判定（`is_publishable_headline`）と同じ基準を使い、公開判定を通らない候補は再試行の対象として選定から除外する。
