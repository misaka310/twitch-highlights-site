...
vod_sources.py remains the source boundary for Twitch and provider dispatch. A focused youtube_sources.py module owns YouTube URL parsing, the Oracle transport contract, and the transient live-chat parser. The YouTube transport is an explicit command-array configuration named YOUTUBE_ORACLE_COMMAND_JSON whose execution must be the existing remote Oracle route; no direct local YouTube chat fallback is allowed. The remote command emits JSON Lines containing optional metadata and live-chat objects with videoOffsetTimeMsec. The parser returns only normalized in-memory offsets and safe metadata.
